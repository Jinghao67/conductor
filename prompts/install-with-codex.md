...
```text
Install the Clean Branch Codex skill from this GitHub repository:

https://github.com/Jinghao67/clean-branch

Goal:
- Install the repository's `skills/clean-branch` folder into my Codex skills directory at `${CODEX_HOME:-~/.codex}/skills/clean-branch`.
- Preserve any existing installed `clean-branch` skill by moving it to a timestamped backup.
...
2. Inspect the repo layout and confirm `skills/clean-branch/SKILL.md` exists.
...
4. If the script is not present, install manually:
   ...
   - back up any existing `${CODEX_HOME:-~/.codex}/skills/clean-branch`
   - copy `skills/clean-branch` into `${CODEX_HOME:-~/.codex}/skills/clean-branch`
